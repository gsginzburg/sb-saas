...
public interface TenantUserRepository extends JpaRepository<TenantUser, TenantUserId> {
    ...
    List<TenantUser> findByTenantIdAndStatus(UUID tenantId, UserStatus status);

    @Query("SELECT tu FROM TenantUser tu WHERE tu.userId = :userId AND tu.status = 'ACTIVE'")
    List<TenantUser> findActiveByUserId(UUID userId);
}
